%% CWSNN on Ionosphere dataset %% 
clear
close all
clc

%%% Read and process the dataset %%%
Ion_data_numerical = load('Ionosphere.txt');
Ionx = Ion_data_numerical(:,2:34);

%%% Parameter settings %%%
C = 2;                             % the number of class
SampleNum = size(Ionx,1);         % the number of samples
F = size(Ionx,2);                 % the number of features
...
D = 12;                             % the number of spiking layer
max_epoch = 100;
L = 25;                            % time interval
...
for n=1:F
    ...
    for h=1:G
        ...
        for j=1:SampleNum
            height = exp(-(Ionx(j,n)-center)^2/(width^2));
            if height>theta
