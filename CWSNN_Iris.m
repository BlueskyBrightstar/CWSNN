%% CWSNN on Iris dataset %% 
clear
close all
clc

%%% Read and process the dataset %%%
Iris_data_numerical = load('Iris_data_numerical.txt');
Irisx = Iris_data_numerical(:,1:4);

%%% Parameter settings %%%
C = 3;                             % the number of class
SampleNum = size(Irisx,1);         % the number of samples
F = size(Irisx,2);                 % the number of features
...
D = 9;                             % the number of spiking layer
max_epoch = 100;
L = 25;                            % time interval
K = 2;                             % the number of synaptic
w = 0.5*rand(F*G,D,K);
tau = 7;

%%% code the inputs %%%%%%%%%%%%%%%%%%%%%
for n=1:F
    MaxValue = max(Irisx(:,n));
    MinValue = min(Irisx(:,n));
    for h=1:G
        center = MinValue+(2*h-3)/2*(MaxValue-MinValue)/(G-2);
        width = 1/beta*(MaxValue-MinValue)/(G-2);
        for j=1:SampleNum
            height = exp(-(Irisx(j,n)-center)^2/(width^2));
            if height>theta
